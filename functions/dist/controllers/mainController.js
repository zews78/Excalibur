...
exports.getCenter = async (req, res) => {
  ...
  try {
    const auth = (await isAuth(req))[0];
    var Cntr = [];
    const CntrRef = firebase.firestore().collection('centres');
...
exports.getOneCenter = async (req, res) => {
  try {
    ...
    const center = await firebase.firestore().collection('centres').doc(req.params.centerId).get();

    // console.log(typeof JSON.parse(dept));
    const userId = req.uid;
    const reqUser = await firebase.firestore().collection('users').doc(req.uid).get();
    // console.log(userId);
...
exports.getOneCenterEymplyeesTicketId = async (req, res) => {
  try {
    const centreId = req.params.centerId;
    const auth = (await isAuth(req))[0];
    const ticketsSnapshot = await firebase.firestore().collection('ticket').where('centre_uid', '==', centreId)
    // .orderBy('timestamp', 'desc')
    .get();
    const reqCentre = await firebase.firestore().collection('centres').doc(centreId).get();
    regUsers = [];
    tickets = [];
...
exports.postTicket = async (req, res) => {
  try {
    const auth = (await isAuth(req))[0];
    let dept = await firebase.firestore().collection('departments').doc(req.body.Department).get();
    let center = await firebase.firestore().collection('centres').doc(req.body.centreId).get();
    let dData = {
      ...dept.data()
    };
    ...
    // console.log(dData);
    const newDept = await firebase.firestore().collection('departments').doc(req.body.Department).set(dData); //overwriting the department document
...
exports.postAvailableSlots = async (req, res) => {
  try {
    const auth = (await isAuth(req))[0];
    let dept = await firebase.firestore().collection('departments').doc(req.body.departmentId).get();
    let dData = {
      ...dept.data()
    };
    let date = req.body.date;
    ...
    dData.bookedSlots.dateExists = false;
    const newDept = await firebase.firestore().collection('departments').doc(req.body.departmentId).set(dData); //overwriting the department document
    ...
    let ticket = await firebase.firestore().collection('ticket').add(tData);
...
exports.getBooked = async (req, res) => {
  try {
    const auth = (await isAuth(req))[0];
    const ticket = await firebase.firestore().collection('ticket').doc(req.params.bookingId).get();
    ...
    const centre = await firebase.firestore().collection('centres').doc(ticket.data().centre_uid).get();
    const user = await firebase.firestore().collection('users').doc(ticket.data().userId).get();
    const department = await firebase.firestore().collection('departments').doc(ticket.data().department).get();
    ...
    const mailOption = {
      ...
      to: 'shariquealam52@gmail.com',
      //////Set the email ID Cannot find it
...
exports.postCenter = async (req, res) => {
  try {
    ...
    if (typeof req.body.department == 'string') {
      department = await firebase.firestore().collection('departments').doc();
      id = department.id;
      // console.log(id);
    ...
    let docRef = await firebase.firestore().collection('centres').add(centerData);
    ...
    index.saveObject(centerData_algolia).then(({
      objectID
    }) => {
      console.log(objectID);
    }).catch(err => {
...
exports.postUser = async (req, res) => {
  try {
    ...
    let user = await firebase.firestore().collection('users').doc(req.params.uid).get();
    let userData = {
      mobile: user.data().mobile
    };
    userData.name = req.body.name;
    userData.password = req.body.psw;
    await firebase.firestore().collection('users').doc(req.params.uid).update(userData);
    res.redirect('/center');
  } catch (e) {
    console.log(e);
  }
...
exports.deleteTicket = async (req, res) => {
  try {
    ...
    const ticket = await firebase.firestore().collection('ticket').doc(ticketId).get();
    let tData = {
      ...ticket.data()
    };
    // console.log(tData.date);
    let dept = await firebase.firestore().collection('departments').doc(tData.department).get();
    let dData = {
      ...dept.data()
    };
    ...
    const newDept = await firebase.firestore().collection('departments').doc(tData.department).set(dData); //overwriting the department document

    const re = await firebase.firestore().collection('ticket').doc(ticketId).delete();
    res.redirect('/center');
  } catch (e) {
    console.log(e);
  }
...
exports.postCategory = async (req, res) => {
  try {
    ...
    let category = await firebase.firestore().collection('category').doc();
